...
void initState() {
  ...
  debugPrint('=== TECHNICIAN ASSIGNMENT DEBUG ===');
  debugPrint('Complaint null check: ${widget.complaint == null}');
  
  if (widget.complaint != null) {
    final complaint = widget.complaint!;
    debugPrint('Complaint ID: ${complaint.complaintId}');
    debugPrint('Complaint Number: ${complaint.complaintNumber}');
    debugPrint('assignedTechnicians length: ${complaint.assignedTechnicians.length}');
    debugPrint('assignedTechnicians data: ${complaint.assignedTechnicians}');
    ...
    if (complaint.assignedTechnicians.isNotEmpty) {
      for (int i = 0; i < complaint.assignedTechnicians.length; i++) {
        ...
        debugPrint('Technician $i details:');
        debugPrint('  - Name: "${tech.name}" (${tech.name.isEmpty ? 'EMPTY' : 'NOT EMPTY'})');
        debugPrint('  - Phone: "${tech.phone}" (${tech.phone.isEmpty ? 'EMPTY' : 'NOT EMPTY'})');
        debugPrint('  - Email: "${tech.email}" (${tech.email.isEmpty ? 'EMPTY' : 'NOT EMPTY'})');
        debugPrint('  - Photo: "${tech.photo}" (${tech.photo?.isEmpty != false ? 'EMPTY/NULL' : 'NOT EMPTY'})');
        debugPrint('  - AssignedAt: "${tech.assignedAt}" (${tech.assignedAt.isEmpty ? 'EMPTY' : 'NOT EMPTY'})');
        debugPrint('  - TechnicianId: "${tech.technicianId}" (${tech.technicianId.isEmpty ? 'EMPTY' : 'NOT EMPTY'})');
      }
    } else {
      debugPrint('No technicians found - list is empty');
    }
  }
  debugPrint('=== END TECHNICIAN ASSIGNMENT DEBUG ===');
    
  complaintImages = widget.complaint?.complaintImages ?? ComplaintImages();
  
  // Fallback to direct images if complaint images are empty
  if (!_hasAnyImages() && (widget.complaint?.images.isNotEmpty == true)) {
    complaintImages = ComplaintImages(tenantUploaded: widget.complaint!.images);
  }
  
  _buildTimelineSteps();
  
  debugPrint('=== TIMELINE DEBUG ===');
  debugPrint('Timeline steps count: ${timelineSteps.length}');
  for (int i = 0; i < timelineSteps.length; i++) {
    debugPrint('Step $i: ${timelineSteps[i].title}');
    debugPrint('  - Has technician: ${timelineSteps[i].technician != null}');
    if (timelineSteps[i].technician != null) {
      debugPrint('  - Technician name: ${timelineSteps[i].technician!.name}');
      debugPrint('  - Technician phone: ${timelineSteps[i].technician!.phone}');
    }
  }
...
Widget build(BuildContext context) {
  ...
  Widget _buildImagesSection() {
    if (!_hasAnyImages()) return SizedBox.shrink();
